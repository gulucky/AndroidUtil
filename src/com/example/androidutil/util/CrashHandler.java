...
//		mDeviceCrashInfo.put(STACK_TRACE, result);
		...
		try {
//			if (Conn.IS_REPORT_ERRER_LOG) {
//				new Thread(new Runnable() {
//					public void run() {
//						try {
//							List<NameValuePair> params = new ArrayList<NameValuePair>();
//							params.add(new BasicNameValuePair("phoneType", "Android"));
//							params.add(new BasicNameValuePair("errorMsg", result));
//							params.add(new BasicNameValuePair("phoneModel", Build.MANUFACTURER + "---" + Build.MODEL));
//							BllHttpPost.sendErrorLog("Home/InsertErrorLog", params);
//						} catch (Exception e) {
//							e.printStackTrace();
//						}
//					}
...
//			}
			...
			String fileName = Util.getInstance()
					.getExtPath() + "/" + Conn.FILE_URL + "/error/" + "crash-" + date + CRASH_REPORTER_EXTENSION;
...
//			mDeviceCrashInfo.store(trace, result);
			trace.flush();
			trace.close();
			return fileName;
		} catch (Throwable e) {
	...
	public static void saveTestLogToFile(String name, String content){
		try {
			String fileName = Util.getInstance().getExtPath() + "/" + Conn.FILE_URL + "/error/" + "test-" + name + CRASH_REPORTER_EXTENSION;
			File file = new File(fileName);
			if (!file.exists()) {
				String newFilePath = fileName.substring(0,
						fileName.lastIndexOf('/'));
